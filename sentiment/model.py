...
class CNN(nn.Module):
    ...
    def forward(self, inp):
        ...
        x = self.fc(x)
        self.dropout_p_y_given_x = x.detach()
        not_drop_x = torch.cat(conv_results, 1)
        not_drop_x = self.fc(not_drop_x)
        self.p_y_given_x = not_drop_x.detach()
        return x
